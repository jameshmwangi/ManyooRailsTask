...
RSpec.describe 'Task model function', type: :model do
  describe 'Validation test' do
    context 'If the task Title is an empty string' do
      it 'Validation fails' do
        task = Task.create(title: '', content: 'My test content.', deadline_on: '2026-12-31', priority: 'medium', status: 'not_started')
    ...
    context 'If the task description is empty' do
      it 'Validation fails' do
        task = Task.create(title: 'Document preparation', content: '', deadline_on: '2026-12-31', priority: 'medium', status: 'not_started')
        expect(task).not_to be_valid
    ...
    context 'If the task Title and description have values' do
      it 'You can register a task' do
        task = Task.create(title: 'Document preparation', content: 'My test content.', deadline_on: '2026-12-31', priority: 'medium', status: 'not_started')
  ...
  describe 'Search function' do
    ...
    let!(:first_task) { FactoryBot.create(:task, title: 'first_task', deadline_on: '2026-12-31', priority: 'medium', status: 'not_started') }
    let!(:second_task) { FactoryBot.create(:task, title: 'second_task', deadline_on: '2026-12-30', priority: 'high', status: 'in_progress') }
    let!(:third_task) { FactoryBot.create(:task, title: 'third_task', deadline_on: '2026-12-29', priority: 'low', status: 'completed') }
